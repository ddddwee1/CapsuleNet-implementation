...
def build_graph():
	inph = tf.placeholder(tf.float32,[None,28*28])
	labh = tf.placeholder(tf.float32,[None,10])

	out = build_model(inph)
	decoded = build_decoder(out,labh)

	with tf.variable_scope('Length'):
		length = tf.sqrt(tf.reduce_sum(tf.square(out),-1))

	with tf.variable_scope('Loss'):
		marg_loss = labh * tf.square(tf.maximum(0.,0.9-length)) + 0.5 * (1-labh) * tf.square(tf.maximum(0.,length-0.1))
		marg_loss = tf.reduce_mean(tf.reduce_sum(marg_loss,1),0)
		recon_loss = tf.reduce_mean(tf.square(inph-decoded))
	...
	with tf.variable_scope('Accuracy'):
		accuracy = M.accuracy(length,tf.argmax(labh,1))

	return inph,labh,recon_loss,marg_loss,train_step,accuracy
